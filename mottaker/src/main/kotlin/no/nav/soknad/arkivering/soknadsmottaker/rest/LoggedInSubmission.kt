...
import no.nav.soknad.arkivering.soknadsmottaker.api.LoggedinSoknadApi
...
@Controller
class LoggedInSubmission(private val innsendingService: InnsendingService): LoggedinSoknadApi {

	private val logger = LoggerFactory.getLogger(javaClass)
	private val secureLogsMarker = MarkerFactory.getMarker("TEAM_LOGS")

	override fun loggedInSubmission(
		...
		innsendingService.publishToLoggedinTopic(innsending.innsendingsId, innsending )

		return ResponseEntity(HttpStatus.OK)
	}

	private fun log(key: String, soknad: Innsending) {
		logger.info("$key: Innlogget, Received request ${maskIdsInInnsending(soknad)}")
		logger.info(secureLogsMarker, "$key: Innlogget, Received request '$soknad'")
	}
